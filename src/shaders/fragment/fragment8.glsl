...
void main(){
  ...
  if(centerFlag == false){
    ...
    uv = (1.5 * gl_FragCoord.xy - uResolution) / min(uResolution.x, uResolution.y);
  }

  uv = waveEffect(uv);

  vec3 color1 = palette2(
    vec3(0.9137, 0.0118, 0.0118),
    vec3(0.7373, 0.4824, 0.0745),
    vec3(0.7725, 0.9882, 0.8392),
    vec3(0.5569, 0.6667, 0.0),
    ring(uv, 0.9)
  );
...
//暗い
vec3 bgColor = vec3(fbm(p) / 2.0);

  bgColor += mix(color1, color2, length(uv) - 0.1);
