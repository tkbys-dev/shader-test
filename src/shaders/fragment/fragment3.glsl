...
//   #ifndef DARK_MODE
//     envOrientation = sphericalMatrix(((vec2(81.5, 119.0) / vec2(187.0)) * 2.0 - vec2(1.0)) * 2.0);
// #else
//     envOrientation = sphericalMatrix((vec2(0.7299465240641712, 0.3048128342245989) * 2.0 - vec2(1.0)) * 2.0);
// #endif
...
//   if (uMouse.z > 0.) {
//       cameraMatrix *= rotX(((uMouse.y / uResolution.y) * 2. - 1.) * 2.);
//     cameraMatrix *= rotZ(((uMouse.x / uResolution.x) * 2. - 1.) * 2.);
//   }
...
// float rand = texture2D(iChannel0, gl_FragCoord.xy / iChannelResolution[0].xy).r;
...
void main()
{
    ...
    for (float disperse = 0.; disperse < MAX_DISPERSE; disperse++) {
        ...
        extinctionDist = 0.;
        wavelength = disperse / MAX_DISPERSE;
   		float rand = texture(iChannel0, gl_FragCoord.xy / iChannelResolution[0].xy).r;
